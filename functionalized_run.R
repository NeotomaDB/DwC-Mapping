...
test_dwc_export <- function(x){
  ...
  output <-  data.frame("dcterms:type"         = query_out$`dcterms:type`,
                        ...
                        decimalLatitude        = round(mean(c(query_out$latN,
                                                        query_out$latS)), 3),
                        decimalLongitude       = round(mean(c(query_out$lonE,
                                                        query_out$lonW)),3),
                        geodeticDatum          = "EPSG:4326",
                        coordinatePrecision    = round(max(c(dms_prec(c(query_out$lonE[1], query_out$latN[1])),
                                                       dms_prec(c(query_out$lonW[1], query_out$latS[1])))),7),
